...
- [ ] 모든 탭이 `mockDailyLogs`, `mockSettings` 직접 import → 서비스 레이어로 교체 필요
- [ ] 입력/저장/수정 기능 구현
- [ ] 탭 간 데이터 동기화
...
- [ ] `lib/types.ts` 생성 — `DailyLog`, `WeeklyLog`, `Settings` 인터페이스를 `mock-data.ts`에서 분리
- [ ] 유틸리티 타입 추가 — `DailyLogInput`, `DailyLogUpdate`(Partial), `SettingsInput`, `SettingsUpdate`
- [ ] `lib/services/daily-log-service.ts` 생성 — `getTodayLog()`, `getDailyLog(date)`, `getRecentDailyLogs(count)`, `upsertDailyLog(date, data)`, `closeDailyLog(date)`
- [ ] `lib/services/weekly-log-service.ts` 생성 — `getWeeklyLog(weekStart)`, `getWeeklyLogs(count)`
- [ ] `lib/services/settings-service.ts` 생성 — `getSettings()`, `updateSettings(data)`, `initializeSettings(data)`
- [ ] `lib/services/stats-service.ts` 생성 — `getLowestWeight()`, `getAvgWeight3d(date)`, `getWeightChange(date)`
- [ ] `lib/utils/date-utils.ts` 생성 — `getDayNumber()`, `getWeekRange()`, `formatDate()`, `isToday()`
- [ ] `lib/mock-data.ts` 정리 — mock 상수만 남기고, 타입은 `lib/types.ts`에서 re-export (기존 import 호환)

### 검증 체크리스트

- [ ] `pnpm build` 성공 (타입 에러 없음)
- [ ] 기존 모든 페이지 정상 렌더링 (import 경로 호환)
- [ ] 서비스 함수가 mock 데이터를 올바르게 반환
...
- [ ] Settings 상태 관리 — React Context (`SettingsProvider`) 생성, `app/(tabs)/layout.tsx`에 Provider 추가
- [ ] `components/settings/settings-form.tsx` — 각 필드에 `onChange` 핸들러 추가, "저장" 버튼 추가
- [ ] 온보딩 데이터 수집 보완 — `onboarding-flow.tsx`에서 누락된 step 데이터(키, 체중, 성별, 수분 목표, 루틴, Intensive Day) state 추가
- [ ] 온보딩 완료 시 — `settingsService.initializeSettings()` 호출 → `defaultTab` 기반 리다이렉트
- [ ] Settings 변경 시 다른 탭에 즉시 반영

### 검증 체크리스트

- [ ] Settings에서 코치 이름 변경 → Home 탭에서 반영 확인
- [ ] 온보딩 완주 → Settings 페이지에서 입력값 확인
...
- [ ] 입력 모달/바텀시트 컴포넌트 생성 — `components/input/input-modal.tsx` (항목별 입력 UI)
  - [ ] 체중: 숫자 (소수점 1자리)
  - [ ] 수분: 0.5L 단위 프리셋 버튼
  - [ ] 운동: Y/N 토글
  - [ ] 아침/점심/저녁: 자유 텍스트
  - [ ] 야식: Y/N 토글
  - [ ] 체력: 여유/보통/피곤 3택
- [ ] `input-chip-list.tsx` — 칩 onClick → 입력 모달 열기, 입력 완료 시 칩 상태 업데이트
- [ ] 입력값 저장 — `dailyLogService.upsertDailyLog(date, data)` 호출
- [ ] 진행률 바 — 실제 입력 완료 수 기반 동적 업데이트
- [ ] `free-text-input.tsx` — 기본 키워드 파싱 ("아침은 샌드위치" → breakfast 업데이트)
- [ ] 마감 버튼 — `closeDailyLog(date)` 호출, 마감 후 수정 차단
- [ ] `date-header.tsx` — 이전/다음 날 이동, 해당 날짜 DailyLog 로드
- [ ] 빈 DailyLog 자동 생성 — 오늘 날짜 로그 없으면 빈 로그 생성

### 검증 체크리스트

- [ ] 체중 입력 → 칩에 "89.2 kg" 표시, 진행률 업데이트
- [ ] 8개 항목 모두 입력 → 마감 버튼 활성화
- [ ] 마감 후 칩 클릭 차단
- [ ] 날짜 이동 → 해당 날짜 데이터 로드
- [ ] `pnpm build` 성공
...
- [ ] `app/(tabs)/home/page.tsx` — `mockDailyLogs`/`mockSettings` → `dailyLogService`/`settingsService` 호출로 교체
- [ ] `DietProgressBanner` — 서비스 데이터 기반 렌더링
- [ ] `InputStatusChips` — 오늘의 실제 입력 상태 반영
- [ ] `CoachOneLiner` — 데이터 소스 교체 (로직은 기존 유지)
- [ ] `WeightMiniGraph` — 서비스에서 14일 데이터 가져오기
- [ ] `DailySummary` — 서비스 데이터 기반 렌더링
- [ ] 빈 상태(신규 유저) — 안내 메시지 표시

### 검증 체크리스트

- [ ] Input에서 체중 입력 → Home에서 변경된 체중 확인
- [ ] Settings 코치 이름 변경 → Home에서 반영
- [ ] 빈 상태 UI 정상 표시
- [ ] `pnpm build` 성공
...
- [ ] `app/(tabs)/log/page.tsx` — mock import → 서비스 호출로 교체
- [ ] `components/log/log-list.tsx` — 서비스 데이터 기반, 날짜 범위 선택 또는 무한 스크롤
- [ ] 주간 요약 카드 — 해당 주 실제 계산값 (avgWeight, exerciseDays, lateSnackCount)
- [ ] 일별 로그 카드 클릭 시 상세 보기 (확장 또는 모달) — 선택적
- [ ] 빈 상태 UI

### 검증 체크리스트

- [ ] Input에서 데이터 입력 → Log에서 해당 일자 로그 확인
- [ ] 주간 요약 수치 일치 확인
- [ ] `pnpm build` 성공
...
- [ ] `app/(tabs)/graph/page.tsx` — mock import → 서비스 호출로 교체
- [ ] `components/graph/weight-chart.tsx` — 기간 선택 (2주/1개월/3개월/전체) 동적 필터링
- [ ] 최저 체중 마커, 목표 체중 기준선 — 실제 데이터 기반 동작
- [ ] 3일 평균선 표시 토글
- [ ] 빈 상태 UI

### 검증 체크리스트

- [ ] Input에서 체중 입력 → Graph에서 최신 포인트 확인
- [ ] 기간 필터 전환 시 차트 정상 업데이트
- [ ] 최저 체중 마커 위치 정확성
- [ ] `pnpm build` 성공
...
- [ ] 자동 계산 — avgWeight3d (3일 평균), weightChange (시작 대비), intensiveDay (역대 최저 기준), day (dietStartDate 기반)
- [ ] 마감 로직 — 마감 시 dailySummary, oneLiner 필드 생성 (템플릿 기반, AI 연동은 이후)
- [ ] 주간 로그 자동 생성 — 일요일 마감 시 WeeklyLog 계산
- [ ] 피드백 생성 — 입력 후 즉시 피드백 (템플릿 기반)
- [ ] 탭 간 데이터 일관성 검증
- [ ] 에러 처리 + 로딩 상태 (스켈레톤 UI)

### 검증 체크리스트

- [ ] 전체 시나리오: 온보딩 → 첫 입력 → 마감 → Home 확인 → Log 확인 → Graph 확인 → Settings 변경
- [ ] 3일 평균 계산 정확성 (수동 검산)
- [ ] Intensive Day 판정 로직 정확성
- [ ] `pnpm build` 성공
- [ ] 모바일 뷰포트 전체 플로우 동작
