...
### 후속 — 앱 시작 요청 전체를 큐 밖으로
마운트 시점에 나가는 요청(홈 초기 데이터·프리페치·자동 마감, 설정·챕터·공지 로드)도 전부
Route Handler 로 옮겼다 (`lib/api/boot-api.ts`, 공용 `lib/api/fetch-json.ts`). 효과:
- 앱 시작 직후 Server Action 큐가 비어 있어, 첫 저장이 앞 요청을 기다리지 않는다.
- 큐에 처리 중인 액션이 있을 때 탭을 옮기면 라우터가 이동 후 현재 페이지 전체를
  한 번 더 받던 동작(needsRefresh)이 더 이상 걸리지 않는다.
- Server Action 으로 남은 건 사용자 동작으로 일어나는 쓰기(저장·마감·설정 변경)뿐.
새 마운트 시점 읽기를 추가할 때는 Server Action 대신 이 경로를 쓸 것.
