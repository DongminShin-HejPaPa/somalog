...
- **3막 동작:** 유지 모드 전환(`mode='maintaining'`) / 새 목표(새 챕터 모달) **2지선다**.
  (구 "나중에"는 유지와 동작이 같아 제거 — 목표 달성 후 새 목표를 안 잡으면 곧 유지이므로 통합.)
...
## ✅ 2단계 완료 (2026-06-17) — A·C·E 마무리

- **A. 챕터 회고 화면**: `/settings/chapters`(서버 컴포넌트, 코드스플릿) — 진행/유지 중 챕터 + 🏆 명예의 전당(`achieved=true`) + 🗂️ 지난 도전 기록. 진입점: 설정 메뉴 + 세레머니 3막 링크 + 홈 재진입 링크(이전 챕터 있을 때, cumulativeDay 재사용 — 쿼리 0).
- **C. 점진적 빌드업**: 게이지 글로우(목표 ≤3kg 임박 시 홈 배너 앰버 글로우+멘트, CSS만) + 마일스톤(−5/−10kg… `decideMilestoneReached` 순수함수+유닛8, `detectMilestone`, 작은 토스트). 목표 달성과 동시면 목표 우선.
- **E. 검증·정합성**: 데이터 초기화/데모 로드가 achievements·diet_chapters까지 리셋. E2E 세레머니 노출/미노출 스펙(`e2e/tests/celebration/goal-ceremony.spec.ts`, 로컬 3 passed). 유닛 총 167.

---

## ✅ 3단계 완료 (2026-06-18) — C 확장: 연속 기록(streak) 마일스톤

- **연속 기록 마일스톤(7/30/100/200/365일):** 마감 시 새 streak 단위 도달이면 작은 토스트("🔥 N일 연속 기록!").
  순수함수 `computeCurrentStreak`(연속일 계산) + `decideStreakMilestone`(중복/스킵 방지) 신설 — **유닛 12 추가**(총 179).
  `achievements.type = 'streak_{N}'`(UNIQUE)로 1회만, `resetAchievements`가 전체 삭제하므로 리셋/데모 정합성 자동.
- **우선순위:** 목표 달성 > 감량 마일스톤(−Nkg) > 연속 기록 마일스톤. 앞 단계가 떴으면 streak 판정/INSERT 건너뜀.
- **속도(검증):** streak 판정은 `detectStreakMilestone`뿐이고 호출 지점은 `actionCloseDailyLog`(마감 액션) 한 곳 — **탭/홈 진입 경로 미접촉(핫패스 영향 0)**.
  추가 쿼리는 `date` 컬럼만 읽는 인덱스(`daily_logs_user_date_idx`) 커버 쿼리 1개 + 업적 1개로, 마감이 이미 await하는 AI 총평 지연에 묻힘.
...
- **할 일:**
  ...
  - 3막 `next-step-sheet`의 "명예의 전당" 진입점을 이 화면에 연결(현재 3막은 유지/새목표 2지선다).
