...
- **달성/미달성 구분(중요):** `diet_chapters`는 **종료된 모든 챕터**를 보관하되 `achieved` 플래그로 구분한다.
  - `achieved = true`(종료 시 체중 ≤ 목표) → **명예의 전당 트로피**.
  - `achieved = false`(중도 새출발/포기) → 트로피 아님. "지난 도전 기록"으로만(누적 일수·이력 보존용). 명예의 전당 UI는 **`achieved=true`만** 렌더.
  - 새 챕터 모달 첫 안내문도 이 분기에 맞춰 "명예의 전당 보관" vs "지난 도전 기록 보관"으로 다르게 노출.
- **할 일:**
